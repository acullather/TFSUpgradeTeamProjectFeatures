    ...
    static class Iteration0
    {
        static void Process(string[] args)
        {
            ...
            try
            {
                ...
                string defaultTemplateName = keyValueArguments.ContainsKey("defaultTemplate") ? keyValueArguments["defaultTemplate"] : string.Empty;
                ...
                using (DeploymentServiceHost deploymentServiceHost = CreateDeploymentServiceHost(configDbConnectionString))
                {
                    ...
                    {
                        RunFeatureEnablement(deploymentServiceHost, project, tfsCollection.InstanceId, defaultTemplateName);
                    }
                }
            }
            catch (Exception ex)
        ...
        private static string GetTfsInstallationDir(string tfsVersion = "14.0")
        {
        ...
        private static void RunFeatureEnablement(DeploymentServiceHost deploymentServiceHost, Project project, Guid instanceId, string defaultTemplateName)
        {
            try
            {
                ...
                using (TeamFoundationRequestContext context = CreateServicingContext(deploymentServiceHost, instanceId))
                {
                    ...
                    ProvisionProjectFeatures(context, project, defaultTemplateName);
                }
        ...
        private static void ProvisionProjectFeatures(TeamFoundationRequestContext context, Project project, string defaultTemplateName)
        {
            ...
            else
            {
                ...
                switch (validProcessTemplateDetails.Count())
                {
                    ...
                    default:
                        ...
                        Console.WriteLine("\t{0}: Multiple valid process templates found. Valid templates: {1}", project.Name, string.Join(",", validProcessTemplateDetails.Select(ptd => ptd.ProcessTemplateDescriptorName)));

                        // Try to configure using default template (if it's valid for this project)
                        var defaultTemplate = validProcessTemplateDetails.FirstOrDefault(ptd => ptd.ProcessTemplateDescriptorName == defaultTemplateName);
                        if (defaultTemplate != null)
                        {
                            ProvisionProject(context, project, projectFeatureProvisioningService, defaultTemplate);
                        }
        ...
        private static void ProvisionProject(TeamFoundationRequestContext context, Project project,
            Server.ProjectFeatureProvisioningService projectFeatureProvisioningService, Server.IProjectFeatureProvisioningDetails projectFeatureProvisioningDetail)
        {
        ...
        private static bool Parse(string[] args, out Dictionary<string, string> keyValueArguments)
        {
            ...
            for (int i = 0; i < args.Length; ++i)
            {
                ...
                {
                    keyValueArguments["defaultTemplate"] = args[i].Substring(3);
                }
                else
                {
                    return false;
        ...
        private static void PrintUsage()
        {
            ...
            sb.AppendLine();
            sb.AppendLine("Usage: features4tfs /c:<collectionUrl> [/d:<connectionString>] [/w:<webConfigPath>] [/t:<templateName>]");
            sb.AppendLine();
            ...
            sb.AppendLine("/t:\t\tDefault template name to use when multiple valid templates are found.");
            sb.AppendLine();
