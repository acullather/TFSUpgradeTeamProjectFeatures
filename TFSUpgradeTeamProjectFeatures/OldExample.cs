...
namespace TFSUpgradeTeamProjectFeatures
{
    class OldExample
    {
        static void Process(string[] args)
        {
            ...
            string tfsVersion = ConfigurationManager.AppSettings["TfsVersion"].ToString();
            ...
            try
            {
                ...
                var tfsConfigurationServerUri = new Uri(tfsRootUrl);
                var configurationServer = TfsConfigurationServerFactory.GetConfigurationServer(tfsConfigurationServerUri);
                ...
                using (var deploymentServiceHost = CreateDeploymentServiceHost(configDbConnectionString, tfsVersion))
                {
                    foreach (TeamProjectCollection tpc in tpcService.GetCollections().OrderBy(tpc => tpc.Name))
                    {
                        ...
                        var witStore = tfsCollection.GetService<WorkItemStore>();
        ...
        private static IVssDeploymentServiceHost CreateDeploymentServiceHost(string configDbConnectionString, string tfsVersion)
        {
            ...
            {
                HostType = TeamFoundationHostType.Application | TeamFoundationHostType.Deployment,
                PlugInDirectory = Path.Combine(GetTfsInstallationDir(tfsVersion), @"Application Tier\TFSJobAgent\Plugins")
            };
            var deploymentServiceHost = DeploymentServiceHostFactory.CreateDeploymentServiceHost(
                    deploymentHostProperties,
                    SqlConnectionInfoFactory.Create(configDbConnectionString, null, null));
        ...
        {
            using (var requestContext = deploymentServiceHost.CreateSystemContext(true))
            {
                var host = requestContext.GetService<TeamFoundationHostManagementService>();
